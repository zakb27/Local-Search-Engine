...
def clean_text(all_text):
    ...
    all_text = all_text.translate(translation_table).lower()
    ...
    #     print(len(word))
    final = [x.text for x in text3.ents]
    return final;
...
def part1():
    ...
    for filename in glob.glob(os.path.join(path, '*.html')):
        ...
        all_text = all_text + " " + title.string + " " + title.string
...
def query(queries):
    ...
    misspelled = spell.unknown(queries)
    
    for word in misspelled:
        queries.remove(word)
        queries.append(spell.correction(word))
    print(queries)
    st = PorterStemmer()
    complete= []
    for i in queries:
        for syn in wordnet.synsets(i):
            for l in syn.lemmas():
                complete.append(l.name())
                complete.append(st.stem(l.name().stem))
...
def part4():
    ...
    doc_freq = json.load(sf)
    sf.close()
    sf = open("tf_idf2.json", "r")
    postings = json.load(sf)
    sf.close()
    sf = open("vocabulary2.json", "r")
