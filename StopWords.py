...
def clean_text(all_text):
    ...
    for i in tokens:
        ...
        if i.lower() not in my_array and len(i) > 2:
            if len(i.lower()) > 13 and ' ' not in i:
                errors += 1
            else:
...
def part2():
    sf = open("postings1.json", "r")
    postings = json.load(sf)
    sf.close()
    sf = open("docfreq1.json", "r")
    doc_freq = json.load(sf)
    sf.close()
    sf = open("docid1.json", "r")
    docid = json.load(sf)
    sf.close()
...
def part4():
    ...
    doc_freq = json.load(sf)
    sf.close()
    sf = open("tf_idf.json", "r")
    postings = json.load(sf)
    sf.close()
    sf = open("vocabulary1.json", "r")
