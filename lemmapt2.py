...
def clean_text(all_text):
    ...
    errors = 0
    my_array = stopwords.words('english')

    final = []

    for i in doc:
        if i.lemma_.lower() not in my_array and len(i) > 2 and len(i.lemma_.lower()) <= 13 :
            final.append(i.lemma_.lower())
...
def write_file(docid, postings, vocabulary, doc_freq):
    sf = open("lem2/DocID2.json", "w", encoding="utf-8")
    json.dump(docid, sf, indent=2)
    sf.close()
    sf = open("lem2/vocabulary2.json", "w", encoding="utf-8")
    json.dump(vocabulary, sf, indent=2)
    sf.close()
    sf = open("lem2/postings2.json", "w", encoding="utf-8")
    json.dump(postings, sf, indent=2)
    sf.close()
    sf = open("lem2/docfreq2.json", "w", encoding="utf-8")
...
def part1():
    ...
    path = ("/Users/zakbrook/Documents/Computer Science Uni/Information Retrieval/Coursework2/ueapeople")
...
def part2():
    sf = open("lem2/postings2.json", "r")
    postings = json.load(sf)
    sf.close()
    sf = open("lem2/docfreq2.json", "r")
    doc_freq = json.load(sf)
    sf.close()
    sf = open("lem2/docid2.json", "r")
    docid = json.load(sf)
    sf.close()
    ...
    for i, values in postings.items():
        ...
        postings[i] = new
    sf = open("lem2/tf_idf2.json", "w", encoding="utf-8")
    json.dump(postings, sf, indent=2)
    sf.close()
...
def part4():
    ...
    doc_freq = json.load(sf)
    sf.close()
    sf = open("lem2/tf_idf2.json", "r")
    postings = json.load(sf)
    sf.close()
    sf = open("lem2/vocabulary2.json", "r")
    ...
    while beep:
        ...
        sf = open("/Users/zakbrook/Documents/Computer Science Uni/Information Retrieval/Coursework2/all_words/postings_all.json", "r")
        postings2 = json.load(sf)
        sf.close()
        sf = open("/Users/zakbrook/Documents/Computer Science Uni/Information Retrieval/Coursework2/all_words/vocabulary_all.json", "r")
